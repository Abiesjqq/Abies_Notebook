...
def calculate_reading_stats(markdown):
    ...
    return total_minutes, chinese_chars, code_lines, english_words
...
def on_page_markdown(markdown, **kwargs):
    ...
    reading_time, chinese_chars, code_lines, english_words = calculate_reading_stats(markdown)

    # Skip only if both languages are essentially absent and there's barely any code
    if chinese_chars < 50 and english_words < 50 and code_lines < 5:
        return markdown

    if english_words >= chinese_chars:
        primary_count = english_words
        primary_label = "词"
    else:
        primary_count = chinese_chars
        primary_label = "字"

    reading_info = f"""!!! info reading-info "阅读信息"
    <span class="twemoji"><svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M12 2A10 10 0 0 0 2 12a10 10 0 0 0 10 10 10 10 0 0 0 10-10h-2a8 8 0 0 1-8 8 8 8 0 0 1-8-8 8 8 0 0 1 8-8V2m6.78 1a.69.69 0 0 0-.48.2l-1.22 1.21 2.5 2.5L20.8 5.7c.26-.26.26-.7 0-.95L19.25 3.2c-.13-.13-.3-.2-.47-.2m-2.41 2.12L9 12.5V15h2.5l7.37-7.38-2.5-2.5Z"></path></svg></span> **{primary_count} {primary_label}**&nbsp;&nbsp;**{reading_time}** 分钟&nbsp;&nbsp;本页总访问量 <span id="busuanzi_value_page_pv">加载中...</span>
"""
