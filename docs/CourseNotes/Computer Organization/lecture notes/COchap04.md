...
实际上不同阶段的时间不同，有些时间有空余。  
流水线通过增加吞吐量来提高速度。每条指令的 latency 没有减少（除了时间最长的指令，其余指令的 latency 增加）

RISCV 指令统一 32 位，适合流水线（每次确定读取指令的长度，可以直接 decode 出寄存器等）

流水线的问题：竞争（hazards）  
结构竞争（structure hazard）：资源重叠  
数据竞争（data hazard）：前面的计算结果是后面的源操作数  
控制竞争（control hazard）：后面指令的执行取决于前面指令的控制

结构竞争：将 data memory 和 instruction memory 分开存储。
...
数据竞争的一种解决：中间插入两级 bubble，使得 sub 的读取在 add 写回之后。  
一个时钟周期先是下降沿再是上升沿。用下降沿写（前半个时钟写）、用上升沿读（后半个时钟读），可使不同指令的读写在同一个时钟周期内完成。

改进：forwarding/bypassing  
上一条指令的计算结果在 EX 或 MEM 后就产生，但是 WB 才写回。可将结果直接传给后面要用的指令。  
如果上一条为运算指令，EX 后产生结果，直接传给下一条的 EX 前，不用插入 bubble；如果上一条为 load 指令，需要插入一级 bubble。
...
一种方法：需要提早直到寄存器比较和目标地址的计算，在 ID 这一级增加硬件，将着两个操作移到 ID。这样中间要插入一级 bubble。  
问题：循环中跳转多，bubble 带来的延迟长；现代流水线更长，计算出结果更晚，延迟更长  
改进：预测跳转的结果，在结果出来前直接取预测的指令。  
（具体预测方法略）

取出第 i+1 条指令时，需要有寄存器存第 i 条指令；第 i+1 条指令执行到 EX 时，也需要寄存器存储第 i 条指令的操作数。

流水线结构（图中所有从右往左的过程会引发竞争）：

![4-5](../resources/4-5.png){style="width:580px"}

流水线操作分为"single-clock-cycle" pipeline diagram 和 "multi-clock-cycle" pipeline diagram。

load 指令写回：load 指令 ID 时将 write address 存在寄存器中，随着 load 指令的执行一步步传到每一步的寄存器中，最终 WB 写回时将 write address 一起传回到 Register file 并写入。

流水线中的控制信号：

![4-6](../resources/4-6.png){style="width:580px"}

ID得到控制信号后，将所有信号按WB、M、EX级分类，用寄存器传到要用的阶段。  
